...
if ~isempty(slind)
    ...
    slrange = 1:nsl; % or if no slices were chosen, keep all slices
    disp(['There are ',num2str(nsl),' total slices!! And I''m keeping them']);
end
%limit FOV?
...
%% Algebraic solution
% interestingly, real/imag components must be reversed for simulated 
...
% reg_siz = 5; linedpts = [
% [LGS,~] = secpass(compdata,GS,reg_siz,linedpts);  
...
varicompare = zeros(nr,nc);%test if solution is the largest variance
cnt = 0;
for r = 1:nr
    for c = 1:nc
        if var_GAS(r,c) <= var_GS(r,c) && var_GAS(r,c) <= var_AS(r,c)
            varicompare(r,c) = 1; cnt = cnt +1;
        end
    end
end
...
if sim
    [maxmg]=imageplotSIM(compdata,cyc,slind,noisSTD,flip,TR,T1,T2,nr,nc,GS,AS,GAS,CS,rowrep,CSgold,mmat,var_GS,var_AS,var_GAS,varicompare);
else
    [maxmg]=imageplotEXP(compdata,cyc,slind,nr,nc,GS,AS,GAS,CS,var_GS,var_AS,var_GAS,varicompare);
end
